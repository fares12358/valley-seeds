...
function NavItem({ item, active, onClick }) {
  ...
  return (
    ...
    >
      <Icon
        size={17}
        className={active ? "text-[#96C422]" : "text-white/50 group-hover:text-white/80"}
      />
...
export default function Sidebar({ mobileOpen, onMobileClose }) {
  ...
  // logout() in AuthContext already does window.location.href = "/dashboard/login"
  const handleLogout = () => {
    onMobileClose?.();
    logout();
  };
      ...
      <div className="border-t border-white/10 px-4 py-4">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-8 h-8 rounded-full bg-[#037338]/40 flex items-center justify-center flex-shrink-0">
            <span className="text-xs font-bold text-[#96C422]">
        ...
        <button
          ...
          className="w-full flex items-center gap-2 text-xs text-white/40 hover:text-white/80
            transition-colors py-1.5 px-2 rounded-lg hover:bg-white/8"
        >
          <LogOut size={14} />
          Sign out
        </button>
