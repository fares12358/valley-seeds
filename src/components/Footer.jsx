...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-gradient-to-b from-[#012a14] to-[#011a0c] text-white z-50">
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[#96C422] to-transparent opacity-60" />
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-[#96C422]/5 rounded-full blur-3xl -translate-y-1/2" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8 mb-12">
          ...
          >
            <div className="flex items-center gap-3 mb-5">
              <div className="relative w-40 h-10 flex items-center justify-center">
                <Image src="/images/logo-white.svg" alt="Valley Seeds Logo" fill className="object-contain" />
              </div>
            </div>
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <motion.a
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#96C422]/20 hover:border-[#96C422]/30 transition-all duration-300"
          ...
          >
            <h4 className="text-sm font-semibold text-white mb-5 flex items-center gap-2">
              <span className="w-4 h-px bg-[#96C422]" />
            ...
            <ul className="space-y-3">
              {fo.links.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-sm text-white/60 hover:text-[#96C422] transition-colors duration-200 flex items-center gap-2 group"
                  >
                    <span className="w-1 h-1 rounded-full bg-white/20 group-hover:bg-[#96C422] transition-colors" />
                    {link.label}
                  </Link>
                </li>
              ))}
            ...
            <ul className="space-y-3">
              {fo.services.map((service, i) => (
                <li key={i} className="text-sm text-white/60 flex items-center gap-2">
                  <span className="w-1 h-1 rounded-full bg-[#96C422]/40" />
                  {service}
                </li>
              ))}
            </ul>
          ...
          >
            ...
            <ul className="space-y-4">
              {fo.contact_items.map((item) => {
                ...
                const content = (
                  <>
                    <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center flex-shrink-0 group-hover:bg-[#96C422]/15 transition-colors">
                      <Icon className="w-3.5 h-3.5 text-[#96C422]" />
                    </div>
                    <div>
                      <div className="text-[10px] text-white/40 uppercase tracking-wider mb-0.5">{item.label}</div>
                      <div className="text-sm text-white/70 group-hover:text-[#96C422] transition-colors">{item.value}</div>
                    </div>
                  </>
                );
                return (
